#ifndef REVERSETREE_H
#define REVERSETREE_H
...
public:
    static void analyze(int max_depth) {
        ...
        std::queue<std::pair<long long, int>> q; // {node, depth}
        ...
        while (!q.empty()) {
            ...
            // Generate children
            // Rule 1: Previous was n / 2 -> therefore next is n * 2
            ...
            // Rule 2: Previous was 3n + 1 -> therefore next is (n - 1) / 3
            // Valid only if (node - 1) % 3 == 0 AND the result is an odd integer > 1
            if ((node - 1) % 3 == 0) {
                long long child2 = (node - 1) / 3;
                if (child2 > 1 && (child2 % 2 != 0)) { // Must be odd > 1
